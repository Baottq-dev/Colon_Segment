...
Porting từ https://github.com/microsoft/Swin-Transformer
"""
...
def window_partition(x, window_size):
    """
    Chia feature map thành các cửa sổ có kích thước không chồng lấp
    Args:
        x: (B, H, W, C)
...
def window_reverse(windows, window_size, H, W):
    """
    Đảo ngược quá trình phân chia cửa sổ
    Args:
        windows: (num_windows*B, window_size, window_size, C)
        window_size (int): kích thước cửa sổ
        H (int): chiều cao của feature map
        W (int): chiều rộng của feature map
    Returns:
        x: (B, H, W, C)
    """
...
class SwinTransformer(BaseModule):
    ...
    def init_weights(self, pretrained=None):
        ...
        if pretrained is not None:
            ...
            # Bỏ qua các tham số không phù hợp
            if 'model' in checkpoint:
                state_dict = checkpoint['model']
            else:
                state_dict = checkpoint
                
            # Kiểm tra và bỏ prefix nếu cần
            if list(state_dict.keys())[0].startswith('backbone.'):
                new_state_dict = {}
                for key, value in state_dict.items():
                    if key.startswith('backbone.'):
                        new_key = key[9:]  # Bỏ 'backbone.'
                        new_state_dict[new_key] = value
                state_dict = new_state_dict
                
            # Load weights với strict=False để bỏ qua các tham số không khớp
